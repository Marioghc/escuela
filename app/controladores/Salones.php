	...
	public function alta(){
	   //Definir los arreglos
	    ...
	    if ($_SERVER['REQUEST_METHOD']=="POST") {
	      ...
	      }else if (!preg_match("/^[A-Z]-\d+$/", $descripcion)) {
            array_push($errores, "La descripción debe tener el formato correcto, por ejemplo: A-101.");
          }
          // Clave
	      if(empty($clave)){
	        array_push($errores,"La clave del salón es requerida.");
	      }else if (!filter_var($clave, FILTER_VALIDATE_INT)) {
             array_push($errores, "La clave debe ser un número entero.");
          }
